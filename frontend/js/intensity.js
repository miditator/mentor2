...
// 🔥 УНИВЕРСАЛЬНАЯ ПРЕМИУМ-КАРТОЧКА ДЛЯ ИНТЕНСИВА
function showIntensityCard(htmlContent, buttonsHtml = '') {
...
function showIntensitySetupMode() {
    ...
    showIntensityCard(`
        <div style="text-align: center;">
            ...
            <div style="background: rgba(0, 0, 0, 0.25); padding: 10px 14px; border-radius: 12px; margin-bottom: 15px; font-size: 13px; color: rgba(255, 255, 255, 0.85); border: 1px solid rgba(255, 255, 255, 0.04);">
...
function loadNextIntensityTask() {
    ...
    apiFetch('/intensity/start', {
        ...
        if (data.success) {
            ...
            showIntensityCard(`<div style="text-align: center;">❌ Ошибка: ${data.error}</div>`);
            document.getElementById('text-input-row').style.display = 'flex';
            window.currentAppMode = 'intensity_setup';
        }
    }).catch(err => {
        if (window.isRateLimitError(err)) return window.showLimitCard();
        showIntensityCard(`<div style="text-align: center;">⚠️ Ошибка сети.</div>`);
        document.getElementById('text-input-row').style.display = 'flex';
        window.currentAppMode = 'intensity_setup';
    });
}
...
function showIntensityPhrase() {
    ...
    if (typeof renderTaskCard === 'function') {
        renderTaskCard(currentTask.translation, intensityState.word, currentTask.rule, currentTask.xray, buttons);
    }
}
    ...
    apiFetch('/intensity/check', {
        ...
        if (data.success) {
            ...
            if (data.is_correct) {
                ...
                if (window.userProfile) window.userProfile.phrases_today = (window.userProfile.phrases_today || 0) + 1;
                ...
                buttonHTML = `
                    <div style="margin-top: 15px; font-size: 13px; color: var(--hint-color); text-align: center; width: 100%;">
                        Переходим дальше... ⏳
                    </div>
                `;

                setTimeout(() => {
                    if (window.currentAppMode === 'intensity_active') nextIntensityStep();
                }, 1500);
            ...
            showIntensityCard(`
                <div style="font-size: 11px; color: var(--hint-color); font-weight: bold; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 1px; text-align: left;">Задание:</div>
...
function showIntensityResult() {
    ...
    apiFetch('/intensity/finish', {
        ...
        if (data.success && data.updated === false) {
            progressMsg = `
                <div style="margin-bottom: 12px; font-size: 13px; color: var(--text-color); text-align: center;">
...
function showIntensityHelp() {
    ...
    apiFetch('/intensity/help', {
        ...
        body: JSON.stringify({
            chat_id: user.id,
            original_phrase: currentTask.translation,
            reference_phrase: currentTask.phrase,
            step: step,
            rule: currentTask.rule,
            target_word: intensityState.word
        })
...
function goToTranslatorForWord(word) {
    ...
    if (translatorInput) translatorInput.value = word;
